...
class Session:
    ...
    def periodic_token_refresh(self):
        ...
        if self.expires_in < 60:
            tokenJson = requests.post(self.token_endpoint,
    ...
    def post(self, uri, post=None, **params):
        ...
        return requests.post(API_URL + Session.process_uri(uri), headers={'Authorization': 'Bearer ' + self.token, "Content-Type": "application/json"},
    ...
    def __init__(self, app_key, auth_endpoint, token_endpoint, secret):
        ...
        tokenJson = requests.post(self.token_endpoint, params={'grant_type': 'authorization_code', 'code': authcode,
                                                               'redirect_uri': REDIR_URI, 'client_id': self.app_key,
                                                               'client_secret': self.secret}).json()
        self.parse_json(tokenJson)
        ...
        self.account_key = \
            requests.get(API_URL + '/' + ME_URI, headers={'Authorization': 'Bearer ' + self.token}).json()[ACCOUNT_KEY]
